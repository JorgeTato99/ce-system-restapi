<?php 
  ...
  $result = $periodoEscolar->read();

  // Get row count
  $num = $result->rowCount();

  // Check if any Periodos Escolares
  if($num > 0 && $validacionUsuario && $usuario->id_rol >= 1) {
    ...
    while($row = $result->fetch(PDO::FETCH_ASSOC)) {
      ...
      // array_push($periodoEscolar_arr['data'], $periodoEscolar_item);
    }
  ...
  }else if($num < 1){ // En caso de que no hayan resultados en la base de datos

    $periodoEscolar->mensajeCapturista = "No existen Periodos Escolares registrados en la base de datos.";
    $periodoEscolar->status = "0";
    $periodoEscolar->errorType = "La base de datos de Alumnos está vacía.";
    echo json_encode(
      array('usuario' => $usuario->nombreUsuario, 'rol' => $usuario->id_rol, 'fechaConsulta' => $fechaActual, 'horaConsulta' => $horaActual, 'message' => $periodoEscolar->mensajeCapturista, 'numeroResultados' => $num, 'resultadoQuery1' => $periodoEscolar->mensaje,'status' => $periodoEscolar->status, 'error' => $periodoEscolar->errorType, 'data' => $periodoEscolar_arr)
    );
