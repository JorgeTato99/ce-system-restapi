<?php 
  ...
  if($control && $validacionUsuario && $usuario->id_rol >= 1){
    ...
    // Salida JSON
    echo json_encode(
      array('usuario' => $usuario->nombreUsuario, 'rol' => $usuario->id_rol, 'fechaConsulta' => $fechaActual, 'horaConsulta' => $horaActual, 'message' => $periodoEscolar->mensajeCapturista, 'resultadoQuery1' => $periodoEscolar->mensaje, 'resultadoQuery2' => $periodoEscolar->mensaje2, 'status' => $periodoEscolar->status, 'error' => $periodoEscolar->errorType)
    );
